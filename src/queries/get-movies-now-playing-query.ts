...
export async function getMoviesNowPlayingQuery(page: string, language: LanguageType, with_genres: string): Promise<ResponseServer<UpcomingMoviesResponse>> {
    const config = {
        method: 'get',
        maxBodyLength: Infinity,
        url: `${urlApi}/api/peli_smith/movies_now_playing?page=${page}&language=${language}&with_genres=${with_genres}`,
        headers: {
